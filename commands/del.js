   ...
   if(color === null  ) color = process.env.color
    ...
    if (args[0] === "sanctions") {
        if(process.env.owner ===message.author.id || db.get(`ownermd.${message.author.id}`) === true) {
            ...
            if(!user) return message.lineReply(`Aucun membre trouvé pour \`${args[1]}\``)
        
      
        
            if (!database || database == []) return message.lineReply(`Aucun membre trouvé avec des sanctions pour \`${args[1] || "rien"}\``)
     
            if (!database.find(data => data.id === id)) return message.lineReply(`Aucune sanctions trouvé pour \`${args[2] || "rien"}\``)
        ...
        db.delete(`info.${message.guild.id}.${user.id}`)
            ...
            message.lineReply(`La sanctions **${args[2]}** a été supprimé`)

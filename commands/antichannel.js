  ...
  let color = db.get(`${process.env.owner}.color`) 
   if(color === null  ) color = process.env.color
        ...
        if(args[0] === "on") {
            ...
            if(db.get(`channels_${message.guild.id}`) === "max") return message.channel.send(`L'antichannel est déjà activé`)
            db.set(`channels_${message.guild.id}`, "max")
            message.channel.send(`L'antichannel est maintenant activé même pour les utilisateurs dans la whitelist`)
        }

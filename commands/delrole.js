  ...
  let color = db.get(`${process.env.owner}.color`) 
   if(color === null  ) color = process.env.color
        ...
        if(process.env.owner ===message.author.id || db.get(`ownermd.${message.author.id}`) === true || db.get(`${message.guild.id}.${message.author.id}.wlmd`) === true) {
            ...
            let wassim = client.config.owners.includes(message.author.id) || data.owners.includes(message.author.id)
    if(wassim) {
            ...
            if (rMember.roles.cache.has(role.id)) return message.channel.send(`1 rôle enlevé à 0 membre`)
    
            if (!rMember.roles.cache.has(role.id)) await rMember.roles.remove(role.id, `Rôle enlevé par ${message.author.tag}`);
      
            message.channel.send(`1 rôle enlevé à 1 membre`)
        ...
        return message.channel.send("1 rôle n'a pas pu être enlevé car il a des permissions dangereuses")
        }
